...
public class LazyMessageSearchIndex extends ListeningMessageSearchIndex {
    ...
    public Iterator<MessageUid> search(MailboxSession session, Mailbox mailbox, SearchQuery searchQuery) throws MailboxException {
        ...
        if (done == null) {
            ...
            synchronized (done) {
                Iterator<MailboxMessage> messages = getFactory().getMessageMapper().findInMailbox(mailbox, MessageRange.all(), FetchType.Full, -1);
    ...
    public void update(Mailbox mailbox, List<UpdatedFlags> updatedFlagsList) throws MailboxException {
        index.update(mailbox, updatedFlagsList);
    }
