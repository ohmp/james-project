...
public interface CassandraAttachmentOwnerDAOContract {
    ...
    CassandraAttachmentOwnerDAO attachmentOwnerDAO();

    @Test
    default void retrieveOwnersShouldReturnEmptyByDefault() {
        assertThat(attachmentOwnerDAO().retrieveOwners(ATTACHMENT_ID).join())
            .isEmpty();
    }

    @Test
    default void retrieveOwnersShouldReturnAddedOwner() {
        attachmentOwnerDAO().addOwner(ATTACHMENT_ID, OWNER_1).join();
    ...
    default void retrieveOwnersShouldReturnAddedOwners() {
        attachmentOwnerDAO().addOwner(ATTACHMENT_ID, OWNER_1).join();
        attachmentOwnerDAO().addOwner(ATTACHMENT_ID, OWNER_2).join();
    ...
    default void retrieveOwnersShouldNotThrowWhenMoreReferencesThanPaging() {
        ...
        IntStream.range(0, referenceCountExceedingPaging)
            .boxed()
            .forEach(i -> attachmentOwnerDAO().addOwner(ATTACHMENT_ID, Username.fromRawValue("owner" + i)).join());
