...
public interface CassandraUserMailboxRightsDAOContract {
    ...
    @Test
    default void saveShouldInsertNewRightEntry() {
        userMailboxRightsDAO().update(MAILBOX_ID, ACLDiff.computeDiff(
            MailboxACL.EMPTY,
            new MailboxACL(new Entry(ENTRY_KEY, RIGHTS))))
            .join();

        assertThat(userMailboxRightsDAO().retrieve(USER_NAME, MAILBOX_ID).join())
            .contains(RIGHTS);
    }
    ...
    default void listRightsForUserShouldReturnEmptyWhenEmptyData() {
        assertThat(userMailboxRightsDAO().listRightsForUser(USER_NAME).join())
            .isEmpty();
    }
