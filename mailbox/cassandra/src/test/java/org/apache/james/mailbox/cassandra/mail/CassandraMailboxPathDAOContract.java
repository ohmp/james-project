...
public interface CassandraMailboxPathDAOContract {
    ...
    default void saveShouldInsertNewEntry() {
        assertThat(mailboxPathDao().save(USER_INBOX_MAILBOXPATH, INBOX_ID).join()).isTrue();

        assertThat(mailboxPathDao().retrieveId(USER_INBOX_MAILBOXPATH).join())
    ...
    default void saveOnSecondShouldBeFalse() {
        assertThat(mailboxPathDao().save(USER_INBOX_MAILBOXPATH, INBOX_ID).join()).isTrue();
        assertThat(mailboxPathDao().save(USER_INBOX_MAILBOXPATH, INBOX_ID).join()).isFalse();
    }
    ...
    default void retrieveIdShouldReturnStoredData() {
        mailboxPathDao().save(USER_INBOX_MAILBOXPATH, INBOX_ID).join();
    ...
    default void getUserMailboxesShouldReturnAllMailboxesOfUser() {
        mailboxPathDao().save(USER_INBOX_MAILBOXPATH, INBOX_ID).join();
        mailboxPathDao().save(USER_OUTBOX_MAILBOXPATH, OUTBOX_ID).join();
        mailboxPathDao().save(OTHER_USER_MAILBOXPATH, otherMailboxId).join();

        List<CassandraIdAndPath> cassandraIds = mailboxPathDao()
    ...
    default void deleteShouldNotThrowWhenEmpty() {
        mailboxPathDao().delete(USER_INBOX_MAILBOXPATH).join();
    }
