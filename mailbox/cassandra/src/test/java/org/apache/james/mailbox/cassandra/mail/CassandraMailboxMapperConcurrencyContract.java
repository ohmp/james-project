...
public interface CassandraMailboxMapperConcurrencyContract {
    ...
    CassandraMailboxMapper mailboxMapper();
    ...
    default void saveShouldBeThreadSafe() throws Exception {
        boolean termination = ConcurrentTestRunner.builder()
            ...
            .build((a, b) -> mailboxMapper().save(new SimpleMailbox(MAILBOX_PATH, UID_VALIDITY)))
            .run()
            .awaitTermination(1, TimeUnit.MINUTES);

        assertThat(termination).isTrue();
        assertThat(mailboxMapper().list()).hasSize(1);
    }
    ...
    default void saveWithUpdateShouldBeThreadSafe() throws Exception {
        ...
        boolean termination = ConcurrentTestRunner.builder()
            .threadCount(THREAD_COUNT)
            .operationCount(OPERATION_COUNT)
            .build((a, b) -> mailboxMapper().save(mailbox))
            .run()
            .awaitTermination(1, TimeUnit.MINUTES);

        assertThat(termination).isTrue();
        List<Mailbox> list = mailboxMapper().list();
