...
public interface CassandraAttachmentDAOV2Contract {
    ...
    @Test
    default void getAttachmentV2ShouldReturnEmptyWhenAbsent() {
        Optional<DAOAttachment> attachment = attachmentDAOV2().getAttachment(ATTACHMENT_ID).join();

        assertThat(attachment).isEmpty();
    }

    @Test
    default void getAttachmentV2ShouldReturnAttachmentWhenStored() {
        ...
        attachmentDAOV2().storeAttachment(daoAttachment).join();

        Optional<DAOAttachment> actual = attachmentDAOV2().getAttachment(ATTACHMENT_ID).join();
