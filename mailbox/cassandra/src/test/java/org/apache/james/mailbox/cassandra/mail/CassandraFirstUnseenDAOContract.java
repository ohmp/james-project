...
public interface CassandraFirstUnseenDAOContract {
    ...
    default void retrieveFirstUnreadShouldReturnEmptyByDefault() {
        assertThat(firstUnseenDAO().retrieveFirstUnread(MAILBOX_ID).join().isPresent())
            .isFalse();
    }

    @Test
    default void addUnreadShouldThenBeReportedAsFirstUnseen() {
        firstUnseenDAO().addUnread(MAILBOX_ID, UID_1).join();

        assertThat(firstUnseenDAO().retrieveFirstUnread(MAILBOX_ID).join())
            .contains(UID_1);
    }
    ...
    default void retrieveFirstUnreadShouldReturnLowestUnreadUid() {
        ...
        firstUnseenDAO().addUnread(MAILBOX_ID, UID_2).join();
    ...
    default void removeUnreadShouldReturnWhenNoData() {
        firstUnseenDAO().removeUnread(MAILBOX_ID, UID_1).join();
    ...
    default void removeUnreadShouldRemoveLastUnread() {
        ...
        firstUnseenDAO().removeUnread(MAILBOX_ID, UID_2).join();
