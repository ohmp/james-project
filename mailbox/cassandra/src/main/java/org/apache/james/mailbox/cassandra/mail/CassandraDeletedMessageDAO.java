...
public class CassandraDeletedMessageDAO {
    ...
    public CompletableFuture<Stream<MessageUid>> retrieveDeletedMessage(CassandraId cassandraId, MessageRange range) {
        CompletableFuture<ResultSet> result;
        switch (range.getType()) {
            case ALL:
                result = retrieveAllDeleted(cassandraId);
                break;
            case FROM:
                result = retrieveDeletedAfter(cassandraId, range.getUidFrom());
                break;
            case RANGE:
                result = retrieveDeletedBetween(cassandraId, range.getUidFrom(), range.getUidTo());
                break;
            case ONE:
                result = retrieveOneDeleted(cassandraId, range.getUidFrom());
                break;
            default:
                throw new UnsupportedOperationException();
        }

        return result.thenApply(CassandraDeletedMessageDAO::resultSetToStream);
    }
