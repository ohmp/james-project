...
public class CassandraACLMapper {
    ...
    public void updateACL(CassandraId cassandraId, MailboxACL.ACLCommand command) throws MailboxException {
        ...
        ACLChange aclChange = updateAcl(cassandraId, aclWithVersion -> aclWithVersion.apply(command), replacement);

        userMailboxRightsDAO.update(cassandraId, aclChange).join();
    }

    public void setACL(CassandraId cassandraId, MailboxACL mailboxACL) throws MailboxException {
        ACLChange aclChange = updateAcl(cassandraId,
    ...
    private ACLChange updateAcl(CassandraId cassandraId, Function<ACLWithVersion, ACLWithVersion> aclTransformation, MailboxACL replacement) throws MailboxException {
        try {
                        ...
                        return getAclWithVersion(cassandraId)
                            .map(value ->
                                updateStoredACL(cassandraId, aclTransformation.apply(value))
                                    .map(newACL -> new ACLChange(value.mailboxACL, newACL)))
                            .orElseGet(() -> insertACL(cassandraId, replacement)
                                .map(newACL -> new ACLChange(MailboxACL.EMPTY, newACL)));
                    });
