...
import org.apache.james.mailbox.acl.ACLChanged;
...
public class CassandraACLMapper {
    ...
    public void updateACL(CassandraId cassandraId, MailboxACL.ACLCommand command) throws MailboxException {
        ...
        ACLChanged aclChanged = updateAcl(cassandraId, aclWithVersion -> aclWithVersion.apply(command), replacement);

        userMailboxRightsDAO.update(cassandraId, aclChanged).join();
    }

    public void setACL(CassandraId cassandraId, MailboxACL mailboxACL) throws MailboxException {
        ACLChanged aclChanged = updateAcl(cassandraId,
    ...
    private ACLChanged updateAcl(CassandraId cassandraId, Function<ACLWithVersion, ACLWithVersion> aclTransformation, MailboxACL replacement) throws MailboxException {
        try {
                ...
                .executeAndRetrieveObject(
                    () -> {
                        ...
                        return getAclWithVersion(cassandraId)
                            .map(value ->
                                updateStoredACL(cassandraId, aclTransformation.apply(value))
                                    .map(newACL -> new ACLChanged(value.mailboxACL, newACL)))
                            .orElseGet(() -> insertACL(cassandraId, replacement)
                                .map(newACL -> new ACLChanged(MailboxACL.EMPTY, newACL)));
                    });
