...
public class CassandraMessageIdToImapUidDAO {
    ...
    public CompletableFuture<Boolean> updateMetadata(ComposedMessageIdWithMetaData composedMessageIdWithMetaData, FlagsUpdateCalculator flagsUpdateCalculator) {
        CassandraId mailboxId = (CassandraId) composedMessageIdWithMetaData.getComposedMessageId().getMailboxId();
        CassandraMessageId messageId = (CassandraMessageId) composedMessageIdWithMetaData.getComposedMessageId().getMessageId();
        ...
        return cassandraAsyncExecutor.executeVoid(
            FlagsUpdateHelper.updateFlags(flagsUpdateCalculator)
                ...
                .and(eq(MESSAGE_ID, messageId.get()))
