...
public class MailboxCopierTest {
    ...
    public void testMailboxCopy() throws MailboxException, IOException {
        ...
        srcMemMailboxManager = new MailboxesAndMessagesFeeder(srcMemMailboxManager).getMockMailboxManager();
    ...
    private void assertMailboxManagerSize(MailboxManager mailboxManager, int multiplicationFactor) throws BadCredentialsException, MailboxException {
        ...
        assertThat(mailboxPathList).hasSize(MailboxesAndMessagesFeeder.EXPECTED_MAILBOXES_COUNT);
        
        for (MailboxPath mailboxPath: mailboxPathList) {
            MessageManager messageManager = mailboxManager.getMailbox(mailboxPath, mailboxSession);
            assertThat(messageManager.getMetaData(false, mailboxSession, FetchGroup.NO_UNSEEN).getMessageCount()).isEqualTo(MailboxesAndMessagesFeeder.MESSAGE_PER_MAILBOX_COUNT * multiplicationFactor);
        }
