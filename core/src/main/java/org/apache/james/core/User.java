...
public class User {
    public static User fromUsername(String username) {
        ...
        int domainDelimiterPosition = username.lastIndexOf('@');
        if (domainDelimiterPosition == -1) {
            return fromLocalPartWithoutDomain(username);
        }
        return fromLocalPartWithDomain(
            username.substring(0, domainDelimiterPosition),
            username.substring(domainDelimiterPosition + 1, username.length()));
    }
