...
Note that the group mail address is virtual: it do not correspond to an existing user.
...
When a specific email is sent to the base mail address, every forward destination address will receive it.
...
Will return the users having forwards configures as a list of JSON Strings representing mail addresses. For instance:
...
Response codes:
 ...
 - 404: The given user don't have forwards does not exist
 - 500: Internal error
...
Will add destination@domain.com to user@domain.com, creating the forwar if needed
...
Response codes:
 ...
 - 403: Server does not own the requested domain
 - 404: Requested forward address do not match an existing user
 - 500: Internal error
