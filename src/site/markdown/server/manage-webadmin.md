...
Once done, we ship code that tries to read from new tables, and if not possible backs up to old tables. You can can thus safely run
...
automatically current schema version use (assess in the database old versions is no more used, as the corresponding tables are empty).
Note the this process is safe: we ensure the service is not running concurrently on this James instance, that it do not bump
...
Response codes:
 ...
 - 410: Error running the migration. This resource is gone away. Reason is metionned in the body.
