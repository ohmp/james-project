...
=== Run James with Java 8 + Guice + Cassandra + ElasticSearch + RabbitMQ
...
If you haven't already:
    ...
    $ docker build -t james/project dockerfiles/compilation/java-8
    $ docker run -v $HOME/.m2:/root/.m2 -v $PWD:/origin \
  -v $PWD/dockerfiles/run/guice/cassandra-rabbitmq/destination:/cassandra-rabbit/destination \
  -t james/project -s HEAD
...
To run this container :
    ...
    $ docker run --hostname HOSTNAME -p "25:25" -p 80:80 -p "110:110" -p "143:143" -p "465:465" -p "587:587" -p "993:993" --link cassandra:cassandra --link elasticsearch:elasticsearch --link rabbitmq:rabbitmq --link tika:tika --name james_run -t james_run
