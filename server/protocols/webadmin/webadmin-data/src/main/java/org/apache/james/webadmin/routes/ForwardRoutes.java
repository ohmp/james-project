...
public class ForwardRoutes implements Routes {
    ...
    public void define(Service service) {
        ...
        service.put(FORWARD_ADDRESS_PATH, (request, response) -> halt(HttpStatus.BAD_REQUEST_400));
        service.put(USER_IN_FORWARD_DESTINATION_ADDRESSES_PATH, this::addToForwardDestinations);
        service.delete(FORWARD_ADDRESS_PATH, (request, response) -> halt(HttpStatus.BAD_REQUEST_400));
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(code = HttpStatus.NOT_FOUND_404, message = "requested base forward address do not match a user"),
        @ApiResponse(code = HttpStatus.INTERNAL_SERVER_ERROR_500,
            message = "Internal server error - Something went bad on the server side.")
    })
    ...
    private void ensureUserExist(MailAddress mailAddress) throws UsersRepositoryException {
        if (!usersRepository.contains(mailAddress.asString())) {
            throw ErrorResponder.builder()
                ...
                .message("Requested base forward address do not correspond to a user")
                .haltError();
        }
    }
