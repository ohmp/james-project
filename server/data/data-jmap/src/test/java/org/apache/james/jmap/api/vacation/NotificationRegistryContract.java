...
public interface NotificationRegistryContract {
    ...
    NotificationRegistry testee();

    @Test
    default void isRegisterShouldReturnFalseByDefault() {
        assertThat(testee().isRegistered(ACCOUNT_ID, recipientId).join()).isFalse();
    }

    @Test
    default void registerShouldWork() {
        testee().register(ACCOUNT_ID, recipientId, Optional.empty()).join();

        assertThat(testee().isRegistered(ACCOUNT_ID, recipientId).join()).isTrue();
    }

    @Test
    default void registerShouldWorkWithExpiracyDate() {
        when(zonedDateTimeProvider().get()).thenReturn(ZONED_DATE_TIME);
        testee().register(ACCOUNT_ID, recipientId, Optional.of(ZONED_DATE_TIME_PLUS_4_SECONDS)).join();
    ...
    default void registerShouldExpireAfterExpiracyDate() {
        ...
        await().atMost(20, TimeUnit.SECONDS).until(() -> !testee().isRegistered(ACCOUNT_ID, recipientId).join());
    }
    ...
    default void registerShouldNotPersistWhenExpiryDateIsPast() {
        ...
        testee().register(ACCOUNT_ID, recipientId, Optional.of(ZONED_DATE_TIME)).join();
