...
public class HBaseRecipientRewriteTable extends AbstractRecipientRewriteTable {
    ...
    protected Mappings mapAddress(String user, Domain domain) throws RecipientRewriteTableException {
        ...
        try {
            table = TablePool.getInstance().getRecipientRewriteTable();
            mappings = getMapping(table, MappingSource.fromUser(user, domain));
            if (mappings == null) {
                mappings = getMapping(table, MappingSource.fromDomain(domain));
            }
            if (mappings == null) {
                mappings = getMapping(table, MappingSource.fromUser(user, Domains.WILDCARD));
            }
        } catch (IOException e) {
