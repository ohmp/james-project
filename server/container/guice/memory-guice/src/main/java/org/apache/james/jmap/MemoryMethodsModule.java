...
public class MemoryMethodsModule extends AbstractModule {
    ...
    protected void configure() {
        ...
        methods.addBinding().to(new TypeLiteral<GetMailboxesMethod<InMemoryId>>(){});
        methods.addBinding().to(new TypeLiteral<GetMessageListMethod<InMemoryId>>(){});
        methods.addBinding().to(new TypeLiteral<GetMessagesMethod<InMemoryId>>(){});
        methods.addBinding().to(new TypeLiteral<SetMessagesMethod<InMemoryId>>(){});
        bind(SetMessagesUpdateProcessor.class).to(new TypeLiteral<SetMessagesUpdateProcessor<InMemoryId>>(){});

        Multibinder<SetMessagesProcessor<InMemoryId>> setMessagesProcessors = Multibinder.newSetBinder(binder(), new TypeLiteral<SetMessagesProcessor<InMemoryId>>(){});
        setMessagesProcessors.addBinding().to(new TypeLiteral<SetMessagesUpdateProcessor<InMemoryId>>(){});
        setMessagesProcessors.addBinding().to(new TypeLiteral<SetMessagesCreationProcessor<InMemoryId>>(){});
        setMessagesProcessors.addBinding().to(new TypeLiteral<SetMessagesDestructionProcessor<InMemoryId>>(){});
    }
