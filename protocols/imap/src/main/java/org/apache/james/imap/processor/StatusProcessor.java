...
public class StatusProcessor extends AbstractMailboxProcessor<StatusRequest> {
    ...
    protected void processRequest(StatusRequest request, ImapSession session, Responder responder) {
        ...
        try {
            ...
            MessageManager.MailboxContentMetaData metaData = retrieveMetadata(mailboxPath, statusDataItems, mailboxSession);
    ...
    private MessageManager.MailboxContentMetaData retrieveMetadata(MailboxPath mailboxPath, StatusDataItems statusDataItems, MailboxSession mailboxSession) throws MailboxException {
        MessageManager mailbox = getMailboxManager().getMailbox(mailboxPath, mailboxSession);
        MessageManager.MailboxContentMetaData.FetchGroup fetchGroup = computeFetchGroup(statusDataItems);
        return mailbox.getMetaData(false, mailboxSession, fetchGroup);
    }

    private MailboxStatusResponse computeStatusResponse(StatusRequest request, StatusDataItems statusDataItems, MessageManager.MailboxContentMetaData metaData) {
    ...
    private MessageManager.MailboxContentMetaData.FetchGroup computeFetchGroup(StatusDataItems statusDataItems) {
        if (statusDataItems.isUnseen()) {
            return MessageManager.MailboxContentMetaData.FetchGroup.UNSEEN_COUNT;
        } else {
            return MessageManager.MailboxContentMetaData.FetchGroup.NO_UNSEEN;
        }
    }

    private Long unseen(StatusDataItems statusDataItems, MessageManager.MailboxContentMetaData metaData) {
        if (statusDataItems.isUnseen()) {
    ...
    private UidValidity uidValidity(StatusDataItems statusDataItems, MessageManager.MailboxContentMetaData metaData) {
    ...
    private ModSeq highestModSeq(StatusDataItems statusDataItems, MessageManager.MailboxContentMetaData metaData) {
    ...
    private MessageUid uidNext(StatusDataItems statusDataItems, MessageManager.MailboxContentMetaData metaData) {
    ...
    private Long recent(StatusDataItems statusDataItems, MessageManager.MailboxContentMetaData metaData) {
    ...
    private Long messages(StatusDataItems statusDataItems, MessageManager.MailboxContentMetaData metaData) {
